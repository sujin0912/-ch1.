...
export class BoardsService {
  ...
  async create(createBoardDto: CreateBoardDto, userId: string): Promise<PostWithRelations> {
  ...
  async update(id: number, updateBoardDto: UpdateBoardDto, userId: string): Promise<PostWithRelations> {
  ...
  try{
    ...
    if(error instanceof
      Prisma.PrismaClientInitializationError && error.code === 'P2025'
    ) {
